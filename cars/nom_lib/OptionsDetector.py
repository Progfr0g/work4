...
class OptionsDetector(ImgGenerator):
    def __init__(self, options={}):
        ...
        self.CLASS_REGION = options.get("class_region",
                                        ["xx_unknown", "eu_ua_2015", "eu_ua_2004", "eu_ua_1995", "eu", "xx_transit",
                                         "ru", "kz"])
    ...
    def prepare(self, base_dir, verbose=1):
        ...
        self.validation_generator = self.compile_test_generator(validation_dir, (self.HEIGHT, self.WEIGHT),
                                                                self.BATCH_SIZE)
    ...
    def train(self, log_dir="./", verbose=1, cnn="simple"):
        ...
        for i in np.arange(self.ENSEMBLES):
            ...
            model = self.create_model(input_model=input_model, conv_base=conv_base,
                                      dropout_1=self.DROPOUT_1, dropout_2=self.DROPOUT_2,
                                      ...
                                      out_dense_activation=self.OUT_DENSE_ACTIVATION,
    ...
    def compile_train_generator(self, train_dir, target_size, batch_size=32):
        ...
        imageGenerator = ImgGenerator(train_dir, self.WEIGHT, self.HEIGHT, self.BATCH_SIZE,
                                      [len(self.CLASS_STATE), len(self.CLASS_REGION), len(self.CLASS_COUNT_LINE)])
    ...
    def compile_test_generator(self, test_dir, target_size, batch_size=32):
        imageGenerator = ImgGenerator(test_dir, self.WEIGHT, self.HEIGHT, self.BATCH_SIZE,
                                      [len(self.CLASS_STATE), len(self.CLASS_REGION), len(self.CLASS_COUNT_LINE)])
