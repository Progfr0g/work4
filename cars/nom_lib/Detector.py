...
class Detector:
    ...
    def __init__(self, mask_rcnn_dir=None, log_dir="./logs/", mask_rcnn_config=None):
        ...
        self.OUTPUT_NODES = (
        "mrcnn_detection/Reshape_1:0", "mrcnn_class/Reshape_1:0", "mrcnn_bbox/Reshape:0", "mrcnn_mask/Reshape_1:0",
    ...
    def loadModel(self, model_path="latest", verbose=0, mode="inference"):
        ...
        import python.cars.nomera.Mask_RCNN.mrcnn.model as modellib
    ...
    def detectFromFile(self, image_paths, verbose=0):
        images = [mpimg.imread(image_path).tolist() for image_path in image_paths]
        return self.detect(images, verbose=verbose)

    def detect(self, images, verbose=0):
        return self.MODEL.detect(self.normalize(images), verbose=verbose)

    def detect_serving(self, images, verbose=0):
        return self.frozen_detect_serving(self.normalize(images), verbose=verbose)
    ...
    def frozen_detect_serving(self, images, verbose=0):
        import python.cars.nomera.Mask_RCNN.mrcnn.model as modellib
        self.MODEL = modellib.MaskRCNN(mode="inference", model_dir=self.LOG_DIR, config=self.CONFIG)
        ...
        molded_images_np = molded_images.copy()
        # anchors = [i.tolist() for i in anchors[0]]
        anchors = anchors[0].tolist()
        # anchors = [i.tolist() for i in anchors]
        image_metas = [i.tolist() for i in image_metas]
        molded_images = molded_images.tolist()
        # print("Run detection")
        ...
        json_response = requests.post("http://localhost:8501/v1/models/number_plates:predict", data=data,
                                      headers=headers)

        predictions = json.loads(json_response.text)['outputs']

        detections = predictions['mrcnn_detection/Reshape_1:0']
        mrcnn_mask = predictions['mrcnn_mask/Reshape_1:0']

        # Process detections
        results = []
        molded_images = molded_images_np

        detections = np.hstack([detections])

        mrcnn_mask = np.hstack([mrcnn_mask])
    ...
    def frozen_detect(self, images, verbose=0):
        ...
        # print("Run detection")
        # Run object detection
        detections, _, _, mrcnn_mask, _, _, _ = self.sess.run(
            [self.mrcnn_detection, self.mrcnn_class, self.mrcnn_bbox, self.mrcnn_mask, self.ROI, self.rpn_class,
